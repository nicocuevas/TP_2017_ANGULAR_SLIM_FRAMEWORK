...
use AppParking\Middleware\CommonMiddlewareUnsafe;
use AppParking\Api\Reusable\EndpointTrait;

$app->group(API_ROUTE, function () use ($app, $entityManager) {

    $app->group('/user-audit', function () use ($app, $entityManager){
        ...
        $app->get('/all[/]', function ($request, $response, $args) use ($entityManager) {
            ...
            $list = $entityManager->getRepository('AppParking\Entity\UserAudit')
        ...
        $app->get('/{id}[/]', function ($request, $response, $args) use ($entityManager) {
            $obj = NULL;
            if(isset($args['id'])){
                $obj = $entityManager->getRepository('AppParking\Entity\UserAudit')
                              ->findById($args['id']);
                if(isset($obj['error'])){
        ...
        $app->post('[/]', function ($request, $response, $args) use ($entityManager){
            ...
            $params = $request->getAttribute('params');
            if($params){
                $em = $entityManager->getRepository('AppParking\Entity\UserAudit');
