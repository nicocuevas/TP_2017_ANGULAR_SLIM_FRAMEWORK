...
$app->group(API_ROUTE, function () use ($app, $entityManager) {
    ...
    $app->group('/vehicle', function () use ($app, $entityManager){
        ...
        $app->get('/all[/]', function ($request, $response, $args) use ($entityManager) {
            ...
            $list = $entityManager->getRepository('AppParking\Entity\Vehicle')
        ...
        $app->get('/{id}[/]', function ($request, $response, $args) use ($entityManager) {
            $obj = NULL;
            if(isset($args['id'])){
                $obj = $entityManager->getRepository('AppParking\Entity\Vehicle')
                              ->findById($args['id']);
                if(isset($obj['error'])){
        ...
        $app->post('[/]', function ($request, $response, $args) use ($entityManager){
            ...
            if($params){
                $em = $em->getRepository('AppParking\Entity\Vehicle');
                $obj = $em->create($params);
                if(isset($obj['error'])){
                   $audit->createSimpleAudit($user->id,"CREATE_VEHICLE_FAIL", json_encode(["params"=>$params],true));
                   return $response->withJson(['response'=>'nok','message'=>$obj['error']], 200);
                }
                $audit->createSimpleAudit($user->id,"CREATE_VEHICLE", json_encode(["params"=>$params],true));
            }else{
                $audit->createSimpleAudit($user->id,"CREATE_VEHICLE_FAIL", json_encode(["params"=>$params],true));
                return $response->withJson(['response'=>'nok'], 400);
            }
        ...
        $app->put('/{id}[/]', function ($request, $response, $args) use ($entityManager){
            ...
            if(isset($args['id']) && $params){
                $em = $entityManager->getRepository('AppParking\Entity\Vehicle');
                $obj = $em->update($args['id'], $params);
                if(isset($obj['error'])){
                   $audit->createSimpleAudit($user->id,"UPDATE_VEHICLE_FAIL", json_encode(["params"=>$params],true));
                   return $response->withJson(['response'=>'nok','message'=>$obj['error']], 200);
                }
                $audit->createSimpleAudit($user->id,"UPDATE_VEHICLE", json_encode(["params"=>$params],true));
            }else{
                $audit->createSimpleAudit($user->id,"UPDATE_VEHICLE_FAIL", json_encode(["params"=>$params],true));
                return $response->withJson(['response'=>'nok'], 400);
            }
